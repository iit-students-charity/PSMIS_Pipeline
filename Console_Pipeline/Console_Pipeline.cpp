...
int main()
{
	...
	if (m <= 0) {
		cout << "\n'm' parameter is zero or less, pipeline wont created";
		return 0;
	}
	...
	while (stepCounter < BinaryNumber::getExpandP()) { 
		...
		if (pipeline.getProcessingPairCounter() < m) {
			cout << "Processing queue:";

			for (auto i = pipeline.getProcessingPairCounter(); i < m; i++) {
				cout << "\n[" << i << "] pair:\n" << pipeline.pairToString(i);
			}
		}
		...
		if (procPairs.size() < m) {
			for (int i = procPairs.size(); i < m; i++) {
				cout << "\n\t(stage -)\t" << i;
